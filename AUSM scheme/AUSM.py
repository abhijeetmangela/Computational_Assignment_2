...
def ausm_flux(qL, qR, nx, ny, gamma):
    """
    Compute AUSM face flux (Euler) for given left/right conservative states
    and face normal (nx, ny), returning the integrated flux A * F_n.
    ...
    def P_split_pos(M, p):
        if abs(M) >= 1.0:
            return 0.5 * p * (1.0 + math.copysign(1.0, M))
        else:
            return 0.25 * p * (M + 1.0)**2 * (2.0 - M)
...
def save_solution(iter_num, rho, rhou, rhov, rhoE, gamma, X, Y):
    ...
    filename = f"solution_roe_iter_{Mach}_{iter_num}_AUSM.csv"
...
plt.title('Convergence History (Roe scheme)')
plt.grid(True, alpha=0.3)
plt.savefig(f'convergence_AUSM_Mach_{Mach}.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.colorbar()
plt.title(f'Pressure (AUSM) (Mach = {Mach})')
plt.xlabel('x')
plt.ylabel('y')
plt.savefig(f'Pressure_Contour_AUSM_Mach_{Mach}.png', dpi=300, bbox_inches='tight')
...
plt.colorbar()
plt.title(f'Velocity Magnitude (AUSM) (Mach = {Mach})')
plt.xlabel('x')
plt.ylabel('y')
plt.savefig(f'Velocity_Magnitude_AUSM_Mach_{Mach}.png', dpi=300, bbox_inches='tight')
...
plt.title(f'Velocity Vectors (AUSM) (Mach = {Mach})')
plt.xlabel('x')
plt.ylabel('y')
plt.tight_layout()
plt.savefig(f'Velocity_Vectors_AUSM_Mach_{Mach}.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.colorbar()
plt.title(f'Mach Numbers (AUSM) (Mach = {Mach})')
plt.xlabel('x')
plt.ylabel('y')
plt.tight_layout()
...
plt.tight_layout()
plt.savefig(f'schlieren_AUSM_Mach_{Mach}.png', dpi=300, bbox_inches='tight')
plt.show()
